...
function simulate(){
 ...
 $('mMedian').textContent=fmt(med);$('mReturn').textContent=`${med>=capital?'+':''}${pct((med/capital-1)*100)} lợi nhuận trung vị`;$('mP95').textContent=fmt(p95);$('mP05').textContent=fmt(p05);$('mProfit').textContent=pct(profit);$('mLoss').textContent=pct(100-profit)+' xác suất thua lỗ';$('mDD').textContent=pct(medDD);$('mStreak').textContent=maxStreak+' lệnh thua';
...
function drawCharts(d){
 ...
 const labels=Array.from({length:d.trades+1},(_,i)=>i), curves=d.pathSample.map((p,i)=>({label:i===0?'Đường mẫu':'',data:p,borderColor:i===0?'#c7ff3d':'rgba(141,189,23,.20)',borderWidth:i===0?2:1,pointRadius:0}));
 const medPath=[];for(let t=0;t<=d.trades;t++){const vals=d.pathSample.map(p=>p[t]);medPath.push(percentile(sampleSorted(vals),.5))}curves.push({label:'Trung vị (mẫu)',data:medPath,borderColor:'#68a7ff',borderWidth:2,pointRadius:0,borderDash:[5,4]});
...
function journal(){const vals=$('tradeInput').value.split(/[,\s]+/).map(Number).filter(Number.isFinite);let b=100,e=[b],peak=b,maxdd=0,w=0,l=0,wl=0,ll=0;vals.forEach(r=>{b+=r;e.push(b);if(r>0){w++;wl++;ll=0}else if(r<0){l++;ll++;wl=0}peak=Math.max(peak,b);maxdd=Math.max(maxdd,(peak-b)/peak*100)});destroy(journalChart);journalChart=new Chart($('journalChart'),{type:'line',data:{labels:e.map((_,i)=>i),datasets:[{label:'Vốn (R)',data:e,borderColor:'#c7ff3d',backgroundColor:'rgba(199,255,61,.08)',fill:true,pointRadius:2,tension:.25}]},options:baseOptions()});const pf=l?Math.abs(vals.filter(x=>x>0).reduce((a,b)=>a+b,0)/vals.filter(x=>x<0).reduce((a,b)=>a+b,0)):Infinity;$('journalStats').innerHTML=`<div><span>Số lệnh</span><b>${vals.length}</b></div><div><span>Net R</span><b>${(b-100).toFixed(2)}R</b></div><div><span>Win rate</span><b>${vals.length?pct(w/vals.length*100):'—'}</b></div><div><span>Profit factor</span><b>${isFinite(pf)?pf.toFixed(2):'∞'}</b></div><div><span>Max drawdown</span><b>${pct(maxdd)}</b></div><div><span>Chuỗi thua dài nhất</span><b>${ll}</b></div>`}
function bind(){
